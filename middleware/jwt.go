...
func Jwt() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if token == "" {
			resp.Fail(response.CodeIllegalLogin, gin.H{"reload": true})
			c.Abort()
			return
		}
		// 按空格分割
		...
		if !(len(parts) == 2 && parts[0] == "seat-token") {
			resp.Fail(response.CodeLoginFailure, gin.H{"reload": true})
			c.Abort()
			return
		}
		jwt := utils.NewJWT()
		...
		if err != nil && err.Error() != "Token is expired" {
			resp.Fail(response.CodeTokenInvalid, gin.H{"reload": true})
			c.Abort()
			return
		}
		//续签
		if mc.ExpiresAt.Unix()-time.Now().Unix() < time.Now().Add(time.Hour*time.Duration(mc.Buffer)).Unix() || err.Error() == "Token is expired" {
			...
			c.Header("new-seat-token", newToken)
